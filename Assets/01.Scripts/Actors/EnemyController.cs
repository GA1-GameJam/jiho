    ...
    IPoolable
{
    ...
    [SerializeField] private Vector2 _flapInterval =
        new(0.55f, 0.95f);

    [SerializeField] private Vector2 _decisionInterval =
        new(0.65f, 1.35f);

    [SerializeField, Range(0f, 1f)]
    private float _trackingChance = 0.72f;

    [SerializeField, Range(0f, 1f)]
    private float _randomFlapChance = 0.22f;
    ...
    private void Die(bool playKillSound)
    {
        ...
        StartCoroutine(
            ReturnAfterDelay(playKillSound));
    }
    ...
    private void TryFlap()
    {
        ...
        if (playerDeltaY > _flapHeightThreshold
            || Random.value < _randomFlapChance)
        {
            Body.AddForce(
